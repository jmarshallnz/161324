...
roll <- read_csv("http://www.massey.ac.nz/~jcmarsha/data/roll_nomacrons.csv")
schools <- read_csv("http://www.massey.ac.nz/~jcmarsha/data/schools.csv")
...
```{r}
# We could do the Total after pivoting. This is OK in this case, but if we had lots of columns to
# add together would be tedious (and error prone - we might miss one by mistake!)
clean %>%
  group_by(EthnicGroup, Gender) %>%
  summarise(Students = sum(Students)) %>%
  pivot_wider(names_from=Gender, values_from=Students) %>%
  mutate(Total = Female + Male)
# Alternatively, compute the total beforehand while the data are tidy
clean %>%
  group_by(EthnicGroup, Gender) %>%
  summarise(Students = sum(Students)) %>%
...
clean %>%
  ...
  summarise(Students = sum(Students)) %>%
  mutate(Total = sum(Students)) %>%
  mutate(Percentage = round(Students / Total * 100, 1)) %>%
  select(-Students, -Total) %>%
...
```{r}
all_data <- clean %>% left_join(schools, by="School")
all_data
```
...
```{r}
all_data %>% filter(EthnicGroup != "International fee paying",
                    Sector == "Secondary",
                    Decile %in% c(1, 10)) %>%
  group_by(Decile, EthnicGroup) %>%
  summarise(Total = sum(Students)) %>%
  ggplot() +
  geom_col(mapping = aes(x = EthnicGroup, y=Total)) +
...
```{r}
imm <- read_excel("immunisation.xlsx", sheet="Ethnicity", skip=3)
imm
```
...
```{r}
imm_cleaner <- imm_cleanish %>% filter(!is.na(DHB), DHB != "National Total")
...
imm_clean %>%
  ...
  ggplot() +
  geom_col(mapping = aes(y = DHB, x = Proportion, fill=GoalReached), alpha=0.9) +
