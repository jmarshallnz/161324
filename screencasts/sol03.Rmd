...
```{r}
# Try replacing the 999 in just the first two numeric columns
yeast_na <- yeast %>%
  replace_with_na(replace = list(alpha.0 = 999,
                                 alpha.7 = 999))
skim(yeast_na)
# Now try doing it all at once
yeast_na <- yeast %>%
...
# We could use a different variable name for this so we can visualise the effect of mean imputation:
husbands_na %>%
  mutate(W.Age.NA = impute_mean(W.Age)) %>%
  ggplot() +
  geom_histogram(mapping = aes(x=W.Age.NA, fill=is.na(W.Age)), bins=10)
```
...
yeast_na %>%
  ...
  arrange(alpha.7_imp) %>%
  ggplot() +
  geom_point(mapping = aes(x=alpha.0, y=alpha.7, col=alpha.7_imp), alpha=0.4)
```
