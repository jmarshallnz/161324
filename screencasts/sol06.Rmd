...
```{r}
#set.seed(1945)
spec2 <- mlp(mode="regression", engine="nnet", hidden_units=11, penalty=0.01,
             epochs = 1000)
fit2 <- spec2 %>% fit(usr ~ ., data=comp_train)
fit2 %>% extract_fit_engine() %>% pluck("convergence")
pred2 <- fit2 %>% augment(new_data=comp_test)
pred2 %>% metrics(truth=usr, estimate=.pred)
```
...
spec2 <- mlp(mode="regression", engine="nnet", hidden_units=11, penalty=0.01,
             epochs = 5000)
fit3 <- spec2 %>% fit(usr ~ ., data=comp_train_scale)
fit3 %>% extract_fit_engine() %>% pluck("convergence")
pred3 <- fit3 %>% augment(new_data=comp_test_scale)
pred3 %>% metrics(truth=usr, estimate=.pred)
```
...
# Create the model
nn_spec <- mlp(mode = "regression", hidden_units = 5, penalty = 0.01,
               epochs = 5000)
...
# Check convergence
nn_fit %>% extract_fit_engine() %>% pluck("convergence")
...
We're getting different metrics from run to run as the initial random seed differs. Initially the weights (of which there are a large number!) are set to random values between -0.5 and 0.5,
and we then optimise the weights from there via back propagation. As there are a large number of weights, there's usually a large number of combinations of those weights that lead to locally optimal (i.e. if you change the weights a little you get a poorer fit) model fits. Essentially we're in very high dimensional space, and there is too much freedom to move about, and lots of potential optimal values for the weights. Finding the 'globally' best set of weights is very hard!
