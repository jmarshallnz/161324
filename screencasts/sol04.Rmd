...
ggplot(comp) + geom_point(mapping=aes(x=lwrite, y=usr))
# We could try a log scale, or maybe a log+1 scale (i.e. log(lwrite+1))
...
```{r}
comp_wide %>% anti_join(comp) # no rows returned: Every row in comp_wide is in comp.
comp %>% anti_join(comp_wide) # no rows returned: Every row in comp is in comp_wide.

# -> comp_wide and comp have the same rows.
# All of the columns in comp are in comp_wide.
# All of the columns in comp_wide are in comp, except the id column.
# -> They must be the same (except for id!)
```
...
```{r}
ggplot(data=comp_long) +
  geom_point(mapping = aes(x=value, y=usr), alpha=0.3) +
  facet_wrap(vars(name), scales = 'free_x')
```

4. What is your conclusion regarding how some of these variables will be useful for predicting `usr`?

Some of these variables, e.g. `pflt`, `scall`, `sread`, `swrite`, `vflt` seem to have information that can be used to predict `usr`, and the relationships seem to be relatively linear (i.e. straight line). So a linear regression should do OK here.
...
```{r}
tidy(split, unique_ind=FALSE) %>% filter(Data == "Assessment")
```
...
```{r}
tidy(split, unique_ind=FALSE) %>% count(Data)
```
...
All the columns other than `usr` are in the model, so that is 21 predictors.

2. Which predictors don't seem to be providing much information?

`lread`, `fork`, `rchar`, `pgout`, `ppgout`, `atch`, `pgin`, don't seem to be important after controlling for the other variables, as their P-values are large.

3. Try using `tidy()` from the `broom()` package to get a tidy summary of the model parameters.

```{r}
tidy(model1)
# this is useful for re-using this information in things (e.g. we could plot the effect sizes)
tidy(model1) %>% filter(p.value > 0.2)
```
...
The `predict()` function can be used for prediction, but it is unpredictable in the type of
output you get. e.g. just run directly on the model you get predictions for the training data as a vector. Run on a new data frame, you get predictions for the new data frame (but what happens with missing values??). If you specify `interval="prediction"` you get a matrix instead of a vector.
```{r}
d <- predict(model1, newdata=comp_test)
length(d)
```

The `augment()` function from `broom` takes a data.frame and *augments* it with the predictions in a predictable way. e.g. it knows how to deal with missing values, so you can be sure the predictions line up properly etc.

```{r}
model1_pred <- augment(model1, newdata = comp_test) %>%
...
```
Interestingly, model2 which is simplier does a better job on the test data than model1 which was more complicated!
...
```{r}
model4_pred <- augment(model4, newdata=comp_test)
model4_pred %>% rmse(truth=usr, estimate=.fitted)
```

The best model appears to be `model2`, which had the lowest root mean square error. That would be the one that we go on with for further modelling.
...
We could find out why (probably!) by looking at the covariates versus the outcome, coloured so we find the odd one. See below in the Bonus section!
...
```{r}
model1_pred %>% filter(usr < 80, .fitted > 90)
model1_pred %>%
  mutate(type = if_else(usr < 80 & .fitted > 90, "problem", "ok")) %>%
  pivot_longer(-c(usr, type, starts_with('.'))) %>%
  ggplot() +
  geom_point(mapping=aes(x=value, y=usr, col=type)) +
  facet_wrap(vars(name), scales = 'free_x')
```
