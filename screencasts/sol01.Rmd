...
You should see there are 103,598 rows for 5 different variables (`School`, `Gender`, `EthnicGroup`, `Level`, `Students`)
...
```{r}
clean %>% summarise(MedStudents = median(Students))
```
...
[^basepipe]: The pipe `%>%` is from the package `magrittr`. This was introduced quite a long time ago, so lots of code you see will use this, and we'll use it as well. There is also a pipe operator in base R, introduced in R v4.1.0, which uses the character combination `|>`. You can use whichever you prefer, and can configure RStudio accordingly so that Ctrl-Shift-M inserts your preferred pipe.
