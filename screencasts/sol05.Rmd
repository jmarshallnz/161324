...
```{r}
comp_pred_rp <- comp_test %>%
  bind_cols(.fitted1 = predict(model1.rp, newdata=comp_test),
            .fitted2 = predict(model2.rp, newdata=comp_test),
            .fitted3 = predict(model3.rp, newdata=comp_test)) %>%
  select(usr, .fitted1, .fitted2, .fitted3, everything())
comp_pred_rp
comp_pred_rp %>%
  summarise(MSE1 = mean((usr - .fitted1)^2),
            MSE2 = mean((usr - .fitted2)^2),
            MSE3 = mean((usr - .fitted3)^2))
```

Alternate to computing the MSE's one by one:

```{r}
comp_pred_rp %>%
  summarise(
    across(
      starts_with(".fitted"), ~ mean((usr - .x)^2)
      )
    )
```
...
```{r}
comp_pred_rp %>%
  rmse(truth=usr, estimate=.fitted1)
comp_pred_rp %>%
  rmse(truth=usr, estimate=.fitted2)
comp_pred_rp %>%
  rmse(truth=usr, estimate=.fitted3)
comp_pred_rp %>%
  mae(truth=usr, estimate=.fitted1)
comp_pred_rp %>%
  mae(truth=usr, estimate=.fitted2)
comp_pred_rp %>%
  mae(truth=usr, estimate=.fitted3)
comp_pred_rp %>%
  metrics(truth=usr, estimate=.fitted1)
comp_pred_rp %>%
  metrics(truth=usr, estimate=.fitted2)
```
...
```{r}
model4.rp <- rpart(usr ~ ., data=comp_train.miss, cp=0.003)
```

2. Plot the tree. You should find that `vflt` is still the first split.

```{r}
plot(model4.rp, margin=0.1)
text(model4.rp)
```
...
If `vflt` is missing, then a surrogate split using `pflt` is used: if `pflt < 92.2` then we go the same way as we would have with `vflt`.
...
```{r}
comp_pred_rf <- comp_test %>%
  bind_cols(.fitted = predict(model.rf, comp_test)) %>%
  select(usr, .fitted, everything())
comp_pred_rf
```
...
```{r}
comp_pred_rf %>%
  summarise(MSE = mean((usr - .fitted)^2))
```
...
```{r}
set.seed(12950)
model2.rf <- randomForest(usr ~ ., data=comp_train, mtry=4, nodesize=1)
comp_pred_rf2 <- comp_test %>%
  bind_cols(.fitted = predict(model2.rf, comp_test)) %>%
  select(usr, .fitted, everything())
comp_pred_rf2 %>%
  summarise(MSE = mean((usr - .fitted)^2))
```
...
```{r}
tree_spec2 <- decision_tree(mode = "regression",
                            cost_complexity = 0.003)

tree_fit2 <- tree_spec2 %>%
  fit(usr ~ ., data = comp_train)

tree_pred2 <- comp_test %>%
  bind_cols(predict(tree_fit2, new_data = comp_test)) %>%
  select(usr, .pred, everything())

tree_pred2 %>% metrics(usr, .pred)
```
...
```{r}
rf_spec <- rand_forest(mode = "regression", engine="randomForest", mtry=3, min_n=2)
rf_spec %>% translate()
