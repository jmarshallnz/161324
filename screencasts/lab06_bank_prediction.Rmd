...
set.seed(1234)
bank_split <- initial_split(bank, prop=3/4)
bank.train <- training(bank_split)
bank.test  <- testing(bank_split)

# Do some EDA: (Lots more than this!)
ggplot(bank) +
  geom_point(mapping=aes(x=popx, y=rejection))

# train a model on bank.train, and then predict bank.test and evaluate the rmse

bank.lm <- linear_reg() %>% fit(rejection ~ ., data=bank.train)
bank.lm %>% tidy()
bank.lm %>% glance()

bank.lm.pred <- bank.lm %>% augment(bank.test)
bank.lm.pred %>% rmse(truth=rejection, estimate=.pred)

bank.base <- linear_reg() %>% fit(rejection ~ 1, data=bank.train)
bank.base %>% tidy()

bank.test %>% mutate(mean_rejection = 0.0424) %>%
  rmse(truth=rejection, estimate=mean_rejection)

bank.rf <- rand_forest(mode='regression', engine='randomForest') %>%
  fit(rejection ~ ., data=bank.train)
bank.rf.pred <- bank.rf %>% augment(bank.test)
bank.rf.pred %>% rmse(truth=rejection, estimate=.pred)

# Ok, the random forest is better than the linear model, so I'm going to go with that.

# Now we need to predict on the real test data. But first, let's re-train our best model
# on all the training data.

bank.final.rf <- rand_forest(mode='regression', engine='randomForest') %>%
  fit(rejection ~ ., data=bank)

bank.final.pred <- bank.final.rf %>%
  augment(bank_predict_me)

# Now I'm going to validate this!
bank.final.pred

# One thing I can do is compare the distribution of the outcome ('rejection', here called '.pred')
# with the distribution on the training data...
ggplot(bank.final.pred) +
  geom_density(mapping=aes(x=.pred))

ggplot(bank) +
  geom_density(mapping=aes(x=rejection))

bind_rows(list(predict = bank.final.pred %>% rename(rejection=.pred),
               train = bank), .id='data') -> all

ggplot(all) +
  geom_histogram(aes(x=rejection, fill=data)) +
  facet_wrap(vars(data), ncol=1, scales='free_y')

bank %>% skimr::skim()
bank.final.pred %>% skimr::skim()

# These look OK, so we'll write out to a .csv file and email Jonathan
bank.final.pred %>% write_csv("JM_bank_prediction.csv")
```
