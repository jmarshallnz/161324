...
`knn` from the `class` library takes the
form
...
We use the familiar `rpart` function.
...
.right-plot[
```{r, ref.label='swiss1', echo=FALSE, message=FALSE}
```
]
...
.right-plot[
```{r, ref.label='swiss2', echo=FALSE, message=FALSE}
```
]
...
swiss.nn <- nnet(type ~ ., size=2, data=swiss.train.scale, maxit=500)
```
