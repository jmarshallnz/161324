...
0.  We generate random initial starting weights.

1.  We 'feed-forward' the training data through the layers to give a prediction $\hat{y}$.

2.  We compute the residual sum of squares RSS. This is our *loss function* that we want to minimise.

3.  RSS is a complicated function of the weights, but it's construction means we can compute the partial derivatives with respect to each weight quite easily (i.e. the direction and amount each weight needs to move to decrease RSS).

4.  We use *batch gradient descent* to move all the weights $w$ a step in the right direction. The step size is known as the *decay* - it's basically to ensure we don't overshoot the minimum RSS.

5.  We take the step, update the weights, then loop back to step 1.
...
-   The argument `linout` is logical, and indicates whether the
    ...
    -   The default setting for `linout` is `FALSE`, when `nnet` produces
        output constrained to the internal $[0,1]$ by applying an additional
        sigmoid activation function.
