...
```{r, message=FALSE}
roll <- read_csv("https://www.massey.ac.nz/~jcmarsha/data/roll.csv")
...
- The teacher finds they don't have all the homework - some is missing.
...
- The `complete.cases` command is useful to use inside `filter` to just get the complete rows.
...
```{r}
husbands_na %>% filter(complete.cases(.)) %>% cor()
```
...
- Replace missing values with modal (i.e. most common) category (level).
...
- We will look at $k$-nearest neighbour imputation, which bases imputed values on records similar to the one requiring imputation.
    ...
    - This is an unlikely value if husband is 25.
    - Better approach is to look at complete cases with husbands that are around 25 years old,
      and use mean of their wives' ages.
...
- We then take the $k$ nearest complete row to our missing row.
...
- There are some sophisticated methods for choosing $k$, but for in many cases $k=5$ is a reasonable rule-of-thumb.
