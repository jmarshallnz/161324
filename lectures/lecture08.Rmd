...
## Introduction to Logistic Regressiond
...
xint <- swiss.pred %>% mutate(lt = .fitted-0.5) %>%
  slice_min(abs(lt), n=2) %>% lm(margin ~ lt, data=.) %>%
...
We have data on from both confirmed cases and those tested but confirmed negative,
...
associated with Gastro symptoms.**
]
...
- Lowering the threshold means we predict more cases (more $y=1$).

- Increasing the threshold means we predict more non-cases (more $y=0$).
...
i.e. correctly identify more cases.
]
