...
We can improve the imputation using numerous methods. A useful one is k-nearest neighbours utilising the `VIM` package.
...
2. Take the `yeast_na` data and `select` out just the columns that start with `alpha`. Use k-nearest neighbour imputation on the this. Take a look at the relationship between `alpha.0` and `alpha.7` as a starting point. For plotting, you might want to add some `alpha` and arrange first by `alpha.7_NA`.
