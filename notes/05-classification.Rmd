...
plot(Alcohol ~ Hue, data=wine, col=Cultivar+1)
```
...
the joint probability function[^2] of $\boldsymbol{x}$ for individuals
...
$\boldsymbol{x}$. Now, by Bayes' Theorem[^3] we have
\begin{equation}
...
$\mu_{11} = \mu_{21} = 0$, and $\mu_{21} = \mu_{22} = 1$. Now, using
standard results for the multivariate normal distribution[^4], we have
$$\begin{align}
f_j(\boldsymbol{x}) &= \frac{1}{2 \pi} \exp \left \{ -\frac {(x_1 - x_{1j})^2}{2} \right \}  \exp \left \{ -\frac{(x_2 - x_{2j})^2}{2} \right \}\\
         &= 
\left \{
\begin{array}{ll}
...
\Rightarrow\quad      x_1^2/2 + x_2^2/2   &<  x_1^2 - 2 x_1 + 1 + x_2^2 - 2 x_2 +1 \\
...
set (with the remaining 78 in the validation dataset). We randomize the
order of the data (to avoid any potential bias in the formation of the
datasets). These operations are performed in the R code below, where we
first convert the target variable `Cultivar` (which is numeric by
default) to a factor.
...
set.seed(2021)
random.order <- sample(1:nrow(wine), size=nrow(wine), replace=FALSE)
wine <- wine[random.order,]
wine.train <- wine[1:100,]
wine.valid <- wine[101:178,]
head(wine.train)
```
...
```{r, message=FALSE}
library(MASS)
wine.lda1 <- lda(Cultivar ~ ., data=wine.train)
wine.lda1
wine.lda1.pred <- predict(wine.lda1, newdata=wine.valid)$class
table(wine.lda1.pred, wine.valid$Cultivar)
```
...
perf.lda1 <- get_perf(wine.lda1.pred, wine.valid$Cultivar)
```

Note the following:

-   We have first loaded the `MASS` library, in order to access the
    `lda` function.
...
-   The `summary` command is not very useful when applied to `lda`
    fitted models. We therefore examine the fitted object `wine.lda1` by
    simply typing the name of the object. Much of the resulting output
...
-   The command `predict(wine.lda1, newdata=wine.valid)` obtains class
    predictions for the observations in `wine.valid`. The resulting
    object is a *list* with components `class`, `posterior` and `x`. The
    first of these contains the class predictions (based on maximum
    posterior probabilities) while the second is a matrix of the
    aforementioned posterior probabilities. The component `x` includes
    the 'score' for each observation on the discriminant variables.

-   The command `table(wine.lda1.pred, wine.valid$Cultivar)` generates a
    cross table of predicted classes against true classes; i.e. the
    confusion matrix. We see that the overall misclassification rate is
...
```{r}
wine.lda2 <- lda(Cultivar ~ Alcohol + Colour + Hue, data=wine.train)
wine.lda2.pred <- predict(wine.lda2,wine.valid)$class
table(wine.lda2.pred,wine.valid$Cultivar)
```

```{r, echo=FALSE}
perf.lda2 <- get_perf(wine.lda2.pred, wine.valid$Cultivar)
```
...
$(\pi_1,\pi_2,\pi_3) = (`r wine.lda1$prior`)$. We can instead specify our
...
prior $(\pi_1,\pi_2,\pi_3) = (1/3,  1/3,  1/3)$ by

```{r, eval=FALSE}
wine.lda3 <- lda(Cultivar ~ ., data=wine.train, prior=c(1,1,1)/3)
```

You might like to see what the classifications are like with this
updated LDA. Given that the prior is only a little different, you should
expect only small changes from the results obtained using `wine.lda1`.
...
displayed in Figure \@ref(fig:circledata). The separation between the red and cyan
...
plot(x2 ~ x1, data=circle.df, pch=19, col=3*as.numeric(y)-1, 
     xlim=c(-2.2,2.2),ylim=c(-2.2,2.2),xlab=expression(x[1]),ylab=expression(x[2]))
```

A (far) more flexible approach than LDA is to estimate the probability
functions $f_1,\ldots,f_C$ using kernel density estimation, as discussed
in section \@ref(sec:kde). Specifically, we will use the training data from
each class to construct kernel estimates $\hat f_1, \ldots, \hat f_C$
when all the predictor variables are continuous. Substituting these
estimates in Equation \@ref(eq:bayes) leads to *kernel discriminant analysis*.
...
typically prove difficult in more than 5 or 6 dimensions, unless there
are very large numbers of observations available.

Kernel discriminant analysis is implemented in R using the `kda`
function within the `ks` library, which must be pre-loaded. The syntax
is a little different for this function to those we have met previously,
being of the form

```{r, eval=FALSE}
kda(x, x.group, eval.points, Hs=...)
```

where the argument `x` takes a matrix of (numerical) predictors for the
training set and `x.group` takes a factor describing class membership
for the observations in the training set. The optional argument `Hs`
defines the bandwidths for each class (the default usually works well,
so there is little reason to supply this argument), while `eval.points`
takes the matrix of predictors for the test set[^5].

The requisite bandwidths can be obtained using the function `Hkda` which
takes arguments `x` and `x.group` as defined above. This function can
take some time to run when applied to large datasets. The functions
`kda` and `Hkda` will not work on data with more than six predictors.
...
```{r}
library(ks)
wine.train.X <- wine.train[,c(2,11,12)]
wine.valid.X <- wine.valid[,c(2,11,12)]
wine.kda <- kda(wine.train.X, wine.train$Cultivar, eval.points=wine.valid.X)
wine.kda$x.group.estimate
table(wine.kda$x.group.estimate, wine.valid$Cultivar)
```

```{r, echo=FALSE}
perf.kda <- get_perf(wine.kda$x.group.estimate, wine.valid$Cultivar)
```

Points to note:

-   The target `Cultivar` is the first column of `wine.train`, and hence
    may be specified as `wine.train[,1]`.

-   The predictors for this problem are columns 2, 11 and 12
    respectively of the training dataset, and hence are specified as
    `wine.train[,c(2,11,12)]`.

-   The output from the `kda` function is a an object containing several
    named variables. The `x.group.estimate` variable is a factor
    containing the predicted classes[^6].
...
wine.nB.1
wine.nB.1.pred <- predict(wine.nB.1, newdata=wine.valid[,2:14])
table(wine.nB.1.pred,wine.valid$Cultivar)
wine.nB.2 <- naiveBayes(Cultivar ~ Alcohol + Colour + Hue, data=wine.train)
wine.nB.2.pred <- predict(wine.nB.2, wine.valid[,c(2,11,12)])
table(wine.nB.2.pred, wine.valid$Cultivar)
```

```{r, echo=FALSE}
perf.nB1 <- get_perf(wine.nB.1.pred, wine.valid$Cultivar)
perf.nB2 <- get_perf(wine.nB.2.pred, wine.valid$Cultivar)
```
...
-   Notice that the test or validation data supplied to the predict
    functions contains only the columns of `wine.valid` corresponding to
...
summary(wine.mn)
wine.mn.pred <- predict(wine.mn, newdata=wine.valid)
table(wine.mn.pred, wine.valid$Cultivar)
```

```{r, echo=FALSE}
perf.mn <- get_perf(wine.mn.pred, wine.valid$Cultivar)
```
...
wine.mn2.pred <- predict(wine.mn2, newdata=wine.valid)
table(wine.mn2.pred, wine.valid$Cultivar)
```

```{r, echo=FALSE}
wine.mn2 <- get_perf(wine.mn2.pred, wine.valid$Cultivar)
```
...
summary(wine.mn3)
wine.mn3.pred <- predict(wine.mn3, newdata=wine.valid)
table(wine.mn3.pred, wine.valid$Cultivar)
```

```{r, echo=FALSE}
wine.mn3 <- get_perf(wine.mn3.pred, wine.valid$Cultivar)
```
...
```{r, message=FALSE}
library(class)
wine.knn.1 <- knn(wine.train[,2:14], wine.valid[,2:14], wine.train$Cultivar, k=5)
wine.knn.1
table(wine.knn.1, wine.valid$Cultivar)
wine.train.scale <- wine.train[,2:14]
wine.valid.scale <- wine.valid[,2:14]
...
wine.valid.scale <- scale(wine.valid.scale, center=wine.means, scale=wine.sds)
wine.knn.2 <- knn(wine.train.scale, wine.valid.scale, wine.train$Cultivar, k=5)
wine.knn.2
table(wine.knn.2, wine.valid$Cultivar)
wine.knn.3 <- knn(wine.train.scale, wine.valid.scale, wine.train$Cultivar, k=10)
table(wine.knn.3, wine.valid$Cultivar)
```

```{r, echo=FALSE}
perf.knn1 <- get_perf(wine.knn.1, wine.valid$Cultivar)
perf.knn2 <- get_perf(wine.knn.2, wine.valid$Cultivar)
```
...
wine.rp.1.prob <- predict(wine.rp.1, newdata=wine.valid)
head(wine.rp.1.prob)
wine.rp.1.pred <- predict(wine.rp.1, newdata=wine.valid, type="class")
table(wine.rp.1.pred, wine.valid$Cultivar)
```
```{r, echo=FALSE}
perf.rp1 <- get_perf(wine.rp.1.pred, wine.valid$Cultivar)
```
...
wine.valid.scale <- wine.valid
wine.train.max <- apply(wine.train[,2:14], 2, max)
wine.train.scale[,2:14] <- sweep(wine.train[,2:14], 2, wine.train.max, FUN="/")
wine.valid.scale[,2:14] <- sweep(wine.valid[,2:14], 2, wine.train.max, FUN="/")
wine.nn.1 <- nnet(Cultivar ~ . , size=7, data=wine.train.scale)
wine.nn.1.prob <- predict(wine.nn.1, newdata=wine.valid.scale)
head(wine.nn.1.prob)
wine.nn.1.pred <- predict(wine.nn.1, newdata=wine.valid.scale, type="class")
table(wine.nn.1.pred, wine.valid$Cultivar)
```

```{r, echo=FALSE}
perf.nn1 <- get_perf(wine.nn.1.pred, wine.valid$Cultivar)
```
...
[^1]: As the names of the first two listed techniques suggest,
...
[^2]: We use the somewhat ambiguous term *probability function* to refer
...
[^3]: Bayes' Theorem is a result of huge importance in Statistics. The
    result is credited to English mathematician and Presbyterian
    minister, Reverend Thomas Bayes (1701?-1760).

[^4]: If you would like to learn more about the multivariate normal
...
[^5]: Note that older versions of the `ks` library use `y` in place of
    `eval.points`, and also require input for the argument `Hs`.

[^6]: In older versions of the `ks` library, the `kda` function returned
    the predicted classes directly, negating the need to append
    `$x.group.estimate`.
