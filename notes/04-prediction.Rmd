...
rect1 <- data.frame(x=c(-1,-1,2.113,2.113), y=c(-0.7,2.9,2.9,-0.7), col=grey(0.95))
rect2 <- data.frame(x=c(2.113,2.113,6.013,6.013), y=c(-0.7,2.9,2.9,-0.7), col=grey(0.85))
rect3 <- data.frame(x=c(6.013,6.013,11,11),y=c(-0.7,2.9,2.9,-0.7), col=grey(0.95))
rects <- bind_rows(lst(rect1, rect2, rect3), .id="rect")
segs <- bind_rows(lst(seg1 = data.frame(x = c(6.013,11), y = 1.794),
                      seg2 = data.frame(x = c(2.113, 6.013), y = 0.5704),
                      seg3 = data.frame(x = c(-1, 2.113), y = 1.645)), .id="seg")
...
1. The tree depth. Typically the trees are not pruned, as we don't mind if the individual trees over-fit as we'll be averaging out any over-fitting. The depth of the tree is generally controlled by restricting splits to nodes with some minimum number of observations.
...
-   In the second neural network we try 5 hidden nodes, giving a total
    ...
    $\textsf{MSE}= `r round(mse.nn$nn.2, 1)`$.
...
-   The third and fourth models differ from the first and second only in
    ...
    gets a little worse ($`r round(mse.nn$nn.3, 1)`$) but the error for the
    network of size 5 improves, to $`r round(mse.nn$nn.4, 1)`$.
:::
