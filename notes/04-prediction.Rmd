...
the late 1980s. For each worker the following attributes are recorded.
  ...
  `AGE`       Age in years
  `RACE`      Ethnicity (Other, Hispanic, or White)
  `OCCUP`     Occupation category (Management, Sales, Clerical, Service, Professional, Other)
  `SECTOR`    Employment sector (Other, Manufacturing, or Construction)
...
library(tidyverse)
wage.train <- read_csv(file="data/wage-train.csv")
```
...
```{r, message=FALSE}
wage.test <- read_csv(file="data/wage-test.csv")
```
...
wage.pred.fit <- wage.pred[,1]
wage.pred.lwr <- wage.pred[,2]
wage.pred.upr <- wage.pred[,3]
head(wage.pred)
```

We can then plot the true target values (i.e. wages) against predictions based
on the linear model `wage.lm`:

```{r wagepred1, fig.cap="Scatterplot of true wages against predicted wages."}
plot(wage.pred.fit, wage.test$WAGE, xlab="Predicted wage", ylab="True wage")
```
...
plot(wage.pred.fit, wage.test$WAGE, xlab="Predicted wage", ylab="True wage",
     ylim=c(-10,30), col=2, pch=19)
segments(wage.pred.fit, wage.pred.lwr, wage.pred.fit, wage.pred.upr, col=grey(0.4))
points(wage.pred.fit, wage.test$WAGE, col=2, pch=19)
```

Some noteworthy points are as follows.

-   The first command saves the predictions in the object `wage.pred`.
    ...
    intervals. We save these columns as vectors `wage.pred.fit`,
    `wage.pred.lwr` and `wage.pred.upr` for later use.

-   The plotting command for the first Figure \@ref(fig:wagepred1)
    requires no special comment, but the
    code to produce Figure \@ref(fig:wagepred2) is a little more involved. We first
    produce the plot of true values against predicted values (using red
    filled circles as plotting symbol). We then use the `segments`
    command to draw line segments from the lower end of each prediction
    interval to the upper. (Look at the help file for `segments` if you
    want to learn more about this function.) The final `points` function
    replots the (red) points in the graph which otherwise get partially
    obscured by the lines. (This is by no means essential, but it does
    make the plot look at little more aesthetically pleasing.)
...
wage.pred.test <- predict(wage.lm, newdata=wage.test)
wage.pred.test.2 <- predict(wage.lm.2, newdata=wage.test)
MSE.test <- mean((wage.pred.test - wage.test$WAGE)^2)
MSE.test.2 <- mean((wage.pred.test.2 - wage.test$WAGE)^2)
MSE.test
MSE.test.2
wage.pred.train <- predict(wage.lm,wage.train)
wage.pred.train.2 <- predict(wage.lm.2,wage.train)
MSE.train <- mean((wage.pred.train - wage.train$WAGE)^2)
MSE.train.2 <- mean((wage.pred.train.2 - wage.train$WAGE)^2)
MSE.train
MSE.train.2
```
...
-   The objects `wage.pred.test` and `wage.pred.test.2` respectively
    contain point predictions for the observations in the test dataset.

-   The observed mean squared error (MSE) of prediction is given by
    ...
    The observed MSE values on the test data using our two models are
    stored as `MSE.test` and `MSE.test.2` respectively. Note that the
    MSE value is a little smaller for the second model, indicating that
    we get slightly better predictions overall using that model. Removal
    of the `RACE` has been beneficial.

-   We can also compute a mean squared error on the training dataset:
    $$\textsf{MSE}_{train} = \frac{1}{n_{train}} \sum_{i=1}^{n_{train}} \left ( y_{train,i} - \hat y_{train,i} \right )^2$$
    using the obvious notation.

    We get $\textsf{MSE}_{train} = `r round(MSE.train, 2)`$ for the first model and
    $\textsf{MSE}_{train} = `r round(MSE.train.2, 2)`$ for the second. Two aspects of these
    results are striking. First, the mean squared errors based on the
...
```{r}
wage.pred.test.step <- predict(wage.lm.step,wage.test)
MSE.test.step <- mean((wage.pred.test.step - wage.test$WAGE)^2)
MSE.test.step
```
...
**You should now do computer lab 5**
...
step is to divide the data into two parts: those with $x < 6.213$ branch
to the left, and those with $x \ge 6.213$ branch to the right. The next
step is to further divide those group with $x < 6.213$ into sub-groups
with $x < 2.163$ and $x \ge 2.163$.

```{r toytree2, echo=FALSE, fig.cap="Regression tree for the toy dataset."}
...
segments(6.213,1.794,11,1.794,lwd=2)
segments(2.163,0.5704,6.213,0.5704,lwd=2)
segments(-1,1.645,2.163,1.645,lwd=2)
box()
```
:::
...
-   The remainder of the summary output describes each node in turn[^24].
    ...
    Observations at levels 1, 3, 5 and 6 (i.e. `Management`, `Clerical`,
    `Professional` and `Other`) are sent down the left-hand branch at
    the split; those at levels 2 and 4 (i.e. `Sales` and `Service`) are
...
```{r}
wage.rp.pred <- predict(wage.rp, newdata=wage.test)
head(wage.rp.pred)
...
```{r wagetree2, fig.cap="Regression trees for the US wage data with complexity parameters $cp=0.1$ (left-hand panel) and $cp=0.001$ right-hand panel) respectively."}
...
You should by now understand the general purpose of most of the R
commands. We adjusted the margins of the plotting regions to reduce
unnecessary white-space and adjusted the size of plotting characters, so
as to improve the appearance of the plots in Figure \@ref(fig:wagetree2), but
such embellishments are hardly critical. The main points to take away
are first, that the choice of $cp=0.1$ leads to a hugely simple tree
...
```{r}
wage.rp.pred.1 <- predict(wage.rp.1, newdata=wage.test)
wage.rp.pred.2 <- predict(wage.rp.2, newdata=wage.test)
mse.rp.1 <- mean((wage.rp.pred.1 - wage.test$WAGE)^2)
mse.rp.2 <- mean((wage.rp.pred.2 - wage.test$WAGE)^2)
mse.rp.1
mse.rp.2
```
...
wage.rp.pred.3 <- predict(wage.rp.3, newdata=wage.test)
mse.rp.3 <- mean((wage.rp.pred.3 - wage.test$WAGE)^2)
mse.rp.3
```
...
curve(1/(1+exp(-x)),from=-5,to=5,xlab="v",ylab=expression(phi(v)))
```
...
wage.nn.pred.1 <- predict(wage.nn.1,wage.test)
mse.nn.1 <- mean((wage.nn.pred.1 - wage.test$WAGE)^2)
mse.nn.1
set.seed(1069)
wage.nn.2 <- nnet(WAGE ~ .,size=5, data=wage.train, linout=TRUE)
wage.nn.pred.2 <- predict(wage.nn.2,wage.test)
mse.nn.2 <- mean((wage.nn.pred.2 - wage.test$WAGE)^2)
mse.nn.2
set.seed(1069)
wage.nn.3 <- nnet(WAGE ~ .,size=8, data=wage.train, linout=TRUE, decay=0.01)
wage.nn.pred.3 <- predict(wage.nn.3,wage.test)
mse.nn.3 <- mean((wage.nn.pred.3 - wage.test$WAGE)^2)
mse.nn.3
set.seed(1069)
wage.nn.4 <- nnet(WAGE ~ .,size=5, data=wage.train, linout=TRUE, decay=0.01)
wage.nn.pred.4 <- predict(wage.nn.4,wage.test)
mse.nn.4 <- mean((wage.nn.pred.4 - wage.test$WAGE)^2)
mse.nn.4
```
...
-   The first model has 8 hidden nodes. Given that there are $p+1$
    ...
    $\textsf{MSE}= `r round(mse.nn.1, 1)`$.

-   In the second neural network we try 5 hidden nodes, giving a total
    of 91 weights to be determined. (Check that you can work out why
    this is the number of weights in the model.) We get
    $\textsf{MSE}= `r round(mse.nn.2, 1)`$.

-   The third and fourth models differ from the first and second only in
    that we specify a value of $0.01$ for the `decay` argument. As a
    result, the prediction error for the neural net with 8 hidden nodes
    gets a little worse ($`r round(mse.nn.3, 1)`$) but the error for the
    network of size 5 improves, to $`r round(mse.nn.4, 1)`$.
:::
...
validation (i.e. artificial test) set.

```{r}
set.seed(442)
validation.index <- sample(1:50,size=10)
validation.index
clock.train.new <- clock.train[-validation.index,]
clock.validation <- clock.train[validation.index,]
```

The object `validation.index` contains the numbers of the 10 training
cases that we randomly select to form the validation set,
`clock.validation`. (We have chosen 10 as a 20% fraction of the original
training data, but there would have been nothing wrong with choosing 20,
say.) The object `clock.train.new` is the new training data, excluding
the validation cases. Note the use of the `set.seed` command to
initialize the random number generator, so that we can later recreate
this precise example if we wish.

We now use the new training data to build a linear regression and
...
clock.pred.lm.valid <- predict(clock.lm, newdata=clock.validation)
clock.pred.lm.valid
clock.pred.lm.valid.MSE <- mean((clock.pred.lm.valid - clock.validation$Price)^2)

library(rpart)
clock.rp <- rpart(Price ~ Age + Condition, data=clock.train.new, cp=0.001)
clock.pred.rp.valid <- predict(clock.rp, newdata=clock.validation)
clock.pred.rp.valid
clock.pred.rp.valid.MSE <- mean((clock.pred.rp.valid - clock.validation$Price)^2)
cbind(clock.validation$Price, clock.pred.lm.valid, clock.pred.rp.valid)
clock.pred.lm.valid.MSE
clock.pred.rp.valid.MSE
```

The third command from the end combines the true sale prices from the
validation set with the predictions using the linear model and
...
squared error values, the linear model appears much the better for
...
clock.pred.test <- predict(clock.lm.final, newdata=clock.test)
data.frame(clock.test$Age, clock.test$Condition, clock.pred.test)
```
...
a real problem (and provides a guide for attempting the assignments). For
Computer Labs 5 and 6 we look at an artificial setting in which we know
the values of the target variable for the test cases, since one of the
purposes of those Labs is to learn more about the comparative
performance of various prediction method methods.

**You should now do Computer Lab 6.**
...
[^28]: Neural networks can also be used to predictor multidimensional
