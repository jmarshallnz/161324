...
plot(Wt.gain ~ Wt.ini, pch=as.numeric(Treatment)^2, col=as.numeric(Treatment),data=goats,xlab="Initial weight",ylab="Weight gain")
legend("topright",legend=c("intensive","standard"),pch=c(1,4),col=1:2)
```
...
scatterhist <-  function(x, y, xlab="", ylab="",x.breaks="Sturges",y.breaks="Sturges"){  
	...
	par(oma=c(3,3,0,0))  
	mtext(xlab, side=1, line=1, outer=TRUE, adj=0, at=0.35)  
	mtext(ylab, side=2, line=1, outer=TRUE, adj=0, at=0.35)
} 
scatterhist(space_comma$Spaces,space_comma$Commas,x.breaks=20,xlab="Number of spaces",ylab="Number of commas")
```
...
package by the end of semester. That's certainly my experience with the
great majority of students. Nonetheless, it requires practice. You'll
find a number of Computer Lab Sheets at the end of this Study Guide.
Don't ignore them -- they are a crucial learning element for this paper.

## Learning Resources{#sec:resources}

The Stream site for the course will be your first port of call to access
learning resources for the paper. It will be updated regularly during
semester, so you should check it on a frequent basis.

Data access is typically done by downloading directly from the web, and
this is usually faciliated for you with code supplied on workshop sheets and
assignment papers.
