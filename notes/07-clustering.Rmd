...
-   Unfortunately, `silhouette()` returns a matrix with class `silhouette`. This cannot
    be coerced directly into a `data.frame` as there is no `data.frame.silhouette()`
    method provided by the `cluster` package. So we do a trick by using `[]` to return
    a plain `matrix`, which can then be coerced into a `data.frame` for plotting.
