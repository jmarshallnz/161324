...
```{r repubstars, echo=FALSE, fig.cap="Stars plot for republican voting data.",fig.height=3}
repub <- read.table("data/repub.txt")
stars(repub, nrow=2, ncol=7, flip.labels = FALSE)
```
...
plot(repub.clust.single)
plot(repub.clust.complete) 
```

The dendrograms are produced are displayed in Figures \@ref(fig:repubsingle) and
...
```{r repubsingle, echo=FALSE, fig.cap="Dendrogram for Republican voting data, obtained using single linkage."}
repub.dist <- dist(repub,"euclidean")
repub.clust.single <- hclust(repub.dist,method="single")
repub.clust.complete <- hclust(repub.dist,method="complete")
par(mar=c(1,4,1,1))
plot(repub.clust.single, xlab="", main="", sub="")
```
```{r repubcomplete, echo=FALSE, fig.cap="Dendrogram for Republican voting data, obtained using complete linkage."}
par(mar=c(1,4,1,1))
plot(repub.clust.complete, xlab="", main="", sub="") 
```
...
-   *Cluster 2*: Alabama, Georgia, Arkansas, Florida, Texas.

-   *Cluster 3*: Mississippi.
...
```{r}
repub.groups.complete <- cutree(repub.clust.complete, h=75)
...
-   Single linkage is prone to 'chaining'. In the figure below the three
    ...
    The left hand cluster in Figure \@ref(fig:repubsingle) in
...
library(cluster)
repub.dist <- dist(repub, method="manhattan")
repub.pam.1 <- pam(repub.dist, 3)
repub.pam.2 <- pam(repub.dist, 4)
...
-   There is a higher average silhouette for 3 clusters rather than 4,
    ...
    the output from silhouette.

    ```{r}
    head(silhouette(repub.pam.2), n=10)
    ```
...
**You should now do Computer Lab 10.**
