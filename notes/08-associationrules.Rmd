...
```{r}
rules <- apriori(Income, parameter=list(support=0.01, confidence=0.6))
rules
summary(rules)
```
...
```{r}
rules <- subset(rules, subset = confidence < 1);
inspect(head(sort(rules, by="confidence"), n=2))
```
...
```{r}
high_income <- subset(rules, subset=rhs %in% "income=$40,000+" & lift > 1.2)
inspect(head(sort(high_income, by="confidence"), n=3))
```
...
```{r}
home_own <- subset(rules, subset=lhs %in% "income=$0-$40,000" & 
                                 rhs %in% "householder status=own" & lift > 1.2)
inspect(head(sort(home_own, by="confidence"), n=2))
```
...
```{r}
new_to_bay <- subset(rules, subset=rhs %in% "years in bay area=1-9" & lift > 1.2)
inspect(head(sort(new_to_bay, by="lift"), n=5))
```
...
**You should now do Computer Lab 11.**
